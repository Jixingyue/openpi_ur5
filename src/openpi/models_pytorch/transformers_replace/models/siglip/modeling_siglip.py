...
def _trunc_normal_(tensor, mean, std, a, b):
    # 从 PyTorch 官方 master 分支拷贝粘贴，直到它进入几个正式发布版本 - RW
    ...
    # 这些值是通过使用一个截断均匀分布，然后对正态分布使用逆 CDF 生成的。
    # 获取 cdf 的上界和下界值
    l = norm_cdf((a - mean) / std)
    u = norm_cdf((b - mean) / std)

    # 用 [l, u] 之间的值均匀填充张量，然后平移至 [2l-1, 2u-1]。
    tensor.uniform_(2 * l - 1, 2 * u - 1)

    # 使用逆 cdf 变换对正态分布进行变换，以得到截断标准正态分布
    tensor.erfinv_()
...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        # 在 tracing 时总是插值，以确保导出的模型能处理动态输入形状
...
class SiglipTextTransformer(nn.Module):
    ...
    def forward(
        ...
        # 注意：与原始 CLIP 模型不同，SigLIP 的文本模型不使用因果掩码。
        # 扩展 attention_mask
        ...
        # 假设采用“粘性”的 EOS 分词，最后一个 token 总是 EOS。
...
class SiglipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # 如果编码器使用 bfloat16，则转换为 bfloat16
...
class SiglipModel(SiglipPreTrainedModel):
    ...
    def get_text_features(
        ...
        ```"""
        # 对于某些字段（如果指定），使用 SigLIP 模型的 config，而不是视觉与文本组件的 config。
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
    ...
    def get_image_features(
        ...
        ```"""
        # 对于某些字段（如果指定），使用 SiglipModel 的 config，而不是视觉与文本组件的 config。
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
    ...
    def forward(
        ...
        loss = None
        if return_loss:
            # 改编自 https://github.com/google-research/big_vision/blob/01edb81a4716f93a48be43b3a4af14e29cdb3a7f/big_vision/trainers/proj/image_text/siglip.py#L287
